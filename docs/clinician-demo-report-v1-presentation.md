# 📄 基於 AI 輔助的泌尿科門診前問診與醫師摘要系統

用途：外部臨床討論用
系統階段：合成資料原型，尚非正式臨床系統
...
下列畫面由合成案例產生，不含真實病人資料。

### 病人 / 家屬端畫面

圖 A：病人/家屬端起始畫面，先用大字、高對比與清楚的開始按鈕降低填答壓力。

![Patient start screen before intake begins](product/screenshots/01-patient-start.png)

圖 B：開始填答後，上方介紹區自動收起，讓題目往頁面上方移動；「今天最想先說哪一個問題？」保留複選與下一步。

![Patient active filling screen with chief concern choices](product/screenshots/02-patient-chief-concern-active.png)

圖 C：「目前對生活造成多大困擾？」改為 1 到 10 的量化程度列，讓長者能直接用大方塊點選痛點程度。

![Patient bother scale with 1 to 10 options](product/screenshots/03-patient-bother-scale.png)

圖 D：進入最後確認時，上方介紹區恢復，病人可以回頭檢查資料來源、症狀重點與送出前內容。

![Patient final confirmation with intro restored](product/screenshots/04-patient-final-confirmation.png)
...
圖 G：同一份填答可依角色拆成病人確認、護理補問與醫師摘要等不同用途畫面。

![Role-separated visit packet export](product/screenshots/07-visit-packet-role-separated.png)

對外討論可聚焦在一個核心問題：

> 這樣的一頁式門診前摘要，是否有機會減少重複問診、提升病史完整度，並且能安全融入實際門診流程？

---

## 1️⃣ 臨床問題與需求背景（Clinical Need）

門診常見的困難，不是單一檢查數值不夠，而是病人進診間前，關鍵病史常常還沒有被有系統地整理好。頻尿、夜尿、急尿、漏尿、排尿困難、血尿、尿痛、發燒畏寒、用藥狀況等資訊，臨床上都很重要，但在實際門診流程中，常會遇到幾個痛點：
...
本原型對應的是門診報到後、醫師看診前的資訊整理流程。設計重點是分工清楚，不把病人端、護理端、醫師端混在同一個畫面。
...
6. 臨床回饋與覆核
   - 醫師與團隊可判斷：摘要格式、欄位內容與安全邊界是否適合進一步討論。

醫師不需要操作問卷，也不需要看完整逐題紀錄；系統的輸出應該是短、可掃讀、可質疑、可追溯來源的摘要。
...
這一段是本系統最重要的可信度基礎：系統的題目與摘要是把 guideline 與臨床問診常用概念，轉成可由病人/家屬先回答的格式。
...
本原型採取保守做法：可以參考 IPSS、ICIQ、bladder diary 等 domains，但目前不宣稱自己產生正式 IPSS 分數、ICIQ 分數或任何 validated score。若未來正式採用完整量表，需確認授權、正式台灣繁中版本、評分方式與臨床覆核。
...
報告中的證據用途是「支持題目域與安全邊界」，不是宣稱本原型已完成臨床效度驗證。
...
- 原型階段只使用合成資料，不輸入真實病人姓名、身分證字號、病歷號、電話、地址或可識別資訊。
...
flowchart TB
  ...
  R5 --> C5["控制：原型只用合成資料<br/>未來需院內資安、個資、IRB 與法遵確認"]
...
## 8️⃣ 治理與導入前提（High-Level Governance）

此版本不揭露內部導入規劃、產品路線或合作策略。若任何機構未來評估臨床使用，至少需先完成以下高階前提：

- 僅在清楚標示「醫師覆核」的前提下使用摘要。
